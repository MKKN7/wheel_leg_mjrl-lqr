...
def _default_gpu_task_mapping(config: Any) -> Mapping[str, Any]:
    ...
    candidates.append(Path(__file__).resolve().parents[2] / "configs" / "warp_curriculum_ppo.yaml")
...
def _validate_batch_contract(batch_config: Any, stage: Any) -> None:
    ...
    if batch_config.safety.torque_fraction_of_rated > batch_config.safety.torque_limit_ratio_sim + 1.0e-9:
        raise WarpCurriculumStageError("batch torque fraction exceeds the configured simulation torque ratio")
