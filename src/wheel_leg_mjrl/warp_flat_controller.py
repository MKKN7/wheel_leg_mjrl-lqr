...
class FixedGainFlatController:
    ...
    def __init__(
        ...
        controller_torque_fraction = float(self.config.max_torque_fraction)
        if controller_torque_fraction > batch_torque_fraction + 1.0e-7:
            raise ValueError(
                "controller max_torque_fraction cannot exceed the batch safety "
                f"torque_fraction_of_rated ({batch_torque_fraction:.3f})"
            )
        # The batch clip is the model-level simulation boundary (up to 100% of
        # rated torque in sim_training).  A controller may deliberately use a
        # lower YAML-owned operating limit, while the batch remains the final
        # independent P0 clip for all residuals and external forces.
        controller_scale = controller_torque_fraction / batch_torque_fraction
        self._controller_control_low = self._control_low * controller_scale
        self._controller_control_high = self._control_high * controller_scale
        if bool(
            (self._nominal_control < self._controller_control_low).any().item()
            or (self._nominal_control > self._controller_control_high).any().item()
        ):
            raise ValueError("calibrated nominal control exceeds the controller torque limit")
        ...
        for gas_dof in np.asarray(calibration.gas_spring_dofs, dtype=np.int64):
            ...
            lower = float(self._controller_control_low[actuator_id].item())
            upper = float(self._controller_control_high[actuator_id].item())
        ...
        self._gas_control_low = (
            self._controller_control_low.index_select(0, self._gas_actuator_ids)
            if self._gas_actuator_ids.numel() > 0
            else torch.empty(0, dtype=torch.float32, device=self.device)
        )
        self._gas_control_high = (
            self._controller_control_high.index_select(0, self._gas_actuator_ids)
            if self._gas_actuator_ids.numel() > 0
            else torch.empty(0, dtype=torch.float32, device=self.device)
        )
        ...
        wheel_cap = float(torch.minimum(
            self._controller_control_high.index_select(0, self._wheel_actuator).abs(),
            self._controller_control_low.index_select(0, self._wheel_actuator).abs(),
        ).min().item())
    ...
    def compute_controls(self, task: Any | None = None) -> Any:
        ...
        torch.clamp(
            self._command,
            min=self._controller_control_low,
            max=self._controller_control_high,
            out=self._command,
        )
        torque_scale = getattr(task, "_controller_torque_scale", None)
        if torque_scale is not None:
