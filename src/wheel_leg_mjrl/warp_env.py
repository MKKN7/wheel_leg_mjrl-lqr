...
def _single_step_parity(batch: WarpPhysicsBatch) -> tuple[float, float, float]:
    ...
    qpos_error = _finite_parity_error(cpu_data.qpos, gpu_qpos, name="qpos")
    qvel_error = _finite_parity_error(cpu_data.qvel, gpu_qvel, name="qvel")
    sensor_error = _finite_parity_error(cpu_data.sensordata, gpu_sensor, name="sensordata")
    batch.reset()
    return qpos_error, qvel_error, sensor_error


def _finite_parity_error(expected: np.ndarray, actual: np.ndarray, *, name: str) -> float:
    """Return a finite max error or fail the preflight before tolerance comparison.

    A comparison such as ``nan > tolerance`` is false, so computing a maximum
    error without an explicit finite check can incorrectly certify a divergent
    CUDA integration.  This helper runs only in the host-side preflight, never
    in the per-world GPU control loop.
    """
    ...
    if expected_array.shape != actual_array.shape:
        raise WarpBatchError(
            f"CPU/GPU one-step parity shape mismatch for {name}: "
            f"cpu={expected_array.shape}, gpu={actual_array.shape}"
        )
    if not np.isfinite(expected_array).all():
        raise WarpBatchError(f"CPU one-step parity state is non-finite: {name}")
    if not np.isfinite(actual_array).all():
        raise WarpBatchError(f"GPU one-step parity state is non-finite: {name}")
    return float(np.max(np.abs(expected_array - actual_array)))
...
def _estop_probe(batch: WarpPhysicsBatch) -> bool:
    ...
    if batch.config.fall_guard.enabled:
        ...
        fall_controls = batch._torch.zeros_like(controls)
        result = batch.step(fall_controls, physics_substeps=1)
        batch._warp.synchronize()
