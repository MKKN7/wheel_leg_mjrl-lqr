...
from guide_wheel_mjcf import load_guide_wheel_model
...
class GuideWheelLqrTest(unittest.TestCase):
    ...
    def test_passive_guide_package_is_complete_and_unactuated(self) -> None:
        configured = load_guide_wheel_model()
        self.assertEqual(len(self.refs.guide_wheel_geoms), len(configured.wheels))
        self.assertEqual(len(self.refs.guide_wheel_joints), len(configured.wheels))
        self.assertEqual(len(configured.wheels), 24)
        self.assertEqual(
            tuple(
        ...
        guide_joint_ids = set(self.refs.guide_wheel_joints)
        driven_joint_ids = set(np.asarray(self.model.actuator_trnid[:, 0], dtype=np.int32).tolist())
        self.assertFalse(guide_joint_ids & driven_joint_ids)
