...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--speed", type=float, default=0.25, help="RMUC-safe walking command magnitude in m/s.")
    parser.add_argument(
        "--turn-rate",
        type=float,
        default=0.20,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Strict RMUC terrain curriculum YAML. Each enabled task/route is reset "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Randomize vehicle dynamics, sensing and delay while keeping RMUC terrain geometry and friction fixed.",
    )
    parser.add_argument(
        "--stochastic-policy",
        action="store_true",
...
def print_summary(
    ...
    *,
    ...
    def print_group(prefix: str, group_results: list[EpisodeResult]) -> None:
    ...
    if route_grouped:
        ...
        for (task, route), route_results in route_grouped.items():
            print_group(f"{task}/{route}", route_results)
    print("task summary")
...
def main() -> None:
    ...
    if args.terrain_curriculum_config is not None:
        ...
        print(
            f"RMUC terrain curriculum: {curriculum.name} stage={stage.stage_id} "
