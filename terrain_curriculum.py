"""Strict, non-navigating task definitions for RMUC locomotion training.
...
class TerrainCurriculumConfig:
    """Immutable validated RMUC training curriculum."""
...
def _task(value: Any, path: str, limits: TerrainCommandLimits) -> TerrainTask:
    mapping = _mapping(value, path)
    _exact_keys(
        mapping,
        {
...
def _stage(value: Any, path: str, known_task_ids: set[str]) -> TerrainCurriculumStage:
    mapping = _mapping(value, path)
    _exact_keys(
        mapping,
        {
...
def load_terrain_curriculum(path: str | Path = DEFAULT_CONFIG_PATH) -> TerrainCurriculumConfig:
    """Load a complete, strict RMUC locomotion curriculum from YAML.
    ...
    _exact_keys(root, {"schema_version", "name", "limits", "tasks", "stages"}, "root")
    schema_version = _integer(root["schema_version"], "root.schema_version", minimum=1)
    if schema_version != SCHEMA_VERSION:
        raise TerrainCurriculumError(
            f"root.schema_version must be {SCHEMA_VERSION}, got {schema_version}"
        )
    ...
    tasks = tuple(
        _task(item, f"root.tasks[{index}]", limits)
    ...
    if actual_task_ids != REQUIRED_TASK_IDS:
        ...
        if unexpected:
            detail.append("unexpected=" + ", ".join(unexpected))
        raise TerrainCurriculumError("root.tasks must contain the RMUC task ids exactly: " + "; ".join(detail))
    stages = tuple(
        _stage(item, f"root.stages[{index}]", set(task_ids))
